...
"""
        ...
        # 构建资金流向描述
        fund_flow_section = ""
        if market_data.get('fund_flow'):
            ff = market_data['fund_flow']
            fund_flow_section = f"""### 💰 资金流向分析
- **机构净流入**：{ff.get('机构', 0)}亿元
- **主力净流入**：{ff.get('主力', 0)}亿元
- **大户净流入**：{ff.get('大户', 0)}亿元
- **散户净流入**：{ff.get('散户', 0)}亿元

> 请重点分析：谁在买？谁在卖？资金是净流入还是净流出？这反映了怎样的市场结构变化？
"""

        prompt = f"""
你是一位拥有 10 年经验的资深宏观策略分析师，风格冷静、客观，擅长透过数据表象看清资金本质。请结合以下【宏观、行业、个股、资金流向】多维数据撰写 Markdown 复盘简报。
...
2. **复盘要点**：
    ...
    - 分析资金流向数据：机构、主力、大户、散户的行为特征，谁在买、谁在卖？
    - 简述汇率/原油宏观因子对今日盘面的影响。
